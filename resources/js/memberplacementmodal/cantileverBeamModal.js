        ...
        $.each(wsDropDown.NoOfBeams, function(index,data) {   
          ...
          $.each(wsDropDown.WeldSize, function(index,data) {   
              ...
              $.each(wsDropDown.Fraction, function(index,data) {   
                  ...
                  $('<option>').val(data.fr).text(data.fr_fra).appendTo('#clbeamlengthfr,#cltosfr,#clfflfr,#distRefgridfr1,#lengthofbeamfr,#lengthofbeamfr1,#clbtosfr,#clbfflfr');
                 
               
            });
              
            $.each(wsDropDown.Inch, function(index,data) {   
                   $('<option>').val(data.Inch).text(data.Inch).appendTo('#clbeamlengthin,#cltosin,#clfflin,#lengthofbeamin,#distRefgridin1,#lengthofbeamin1,#clbtosin,#clbfflin');
            });
		   ...
		   getProfileGrades(profilename);
			 $.each(profileGrades, function(index,data) {   
				  var grades="<option value=\""+data.Grade+"\">"+data.Grade+"</option>";
		           ...
		           $(grades).appendTo('#cantilevermaterialgrade').prop('selected', true);
		   ...
		   if(wsPlateGrade != ''){
		     	...
		     	if(res=="W"){
		     		if(profileshgrade != null){
		     			  $('#cantilevermaterialgrade').val(profileshgrade).prop('selected', true);;
			           }
				   }
		    	else if(res=="H"){
		     	if(hssgrade != ''){
		     		  $('#cantilevermaterialgrade').val(hssgrade).prop('selected', true);;
		         }
		     	} 
		     	else{
		     		 if(cmcgrade != ''){
		     			  $('#cantilevermaterialgrade').val(cmcgrade).prop('selected', true);;
			         } 
		     	}
		     }
...
//Bind Connection Type
  ...
  $.each(wsPlateGrade, function(key, value) {   
		if (value["ShearStudDia"]||value["ShearStudGrade"]||value["ShearStudLength"]) {
			  ...
			  $("#cshear1").val(value["ShearStudDia"]).trigger("change");
			  $("#cshear2").val(value["ShearStudLength"]).trigger("change");
			//  $("#cshear3").val(value["ShearStudGrade"]).trigger("change");
