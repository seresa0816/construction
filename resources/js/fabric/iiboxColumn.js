var class_iibox_Column 	= function(parent)
{
	var main 	= this;

	main.mode 		 = "builtUpCRColumn";
	main.parent  	 = parent;
	...
	main.member_boxColumn = new builtUpCR();
	...
	main.initEvent = function()
	{
		canvas.on("mouse:down", function(evt)
		{
			...
			var boxProperties = new_configure.crucified;
			Object.keys(boxProperties.memberProperties).map(function(entry)
			{
				main.member_boxColumn.memberProperties[entry] = $("#builtupcolumnModal3 #" + boxProperties.memberProperties[entry]).val();
			});

			Object.keys(boxProperties.finishProperties).map(function(entry)
			{
				if (entry != "paintType" && entry != "primerCheck")
					main.member_boxColumn.finishProperties[entry] = $("#builtupcolumnModal3 #" + boxProperties.finishProperties[entry]).val();
				else if (entry == "primerCheck")
					main.member_boxColumn.finishProperties[entry] = $("#builtupcolumnModal3 #" + boxProperties.finishProperties[entry]).prop("checked");
				else
			...
			{
				if (entry == "cap_check")
					main.member_boxColumn.connectionProperties[entry] = $("#builtupcolumnModal3 #" + boxProperties.connectionProperties[entry]).prop("checked");
				else
					main.member_boxColumn.connectionProperties[entry] = $("#builtupcolumnModal3 #" + boxProperties.connectionProperties[entry]).val();
			});
            ...
            for (var i = 0; i < splice_count; i ++) {
                ...
                splice.sign = $("#builtupcolumnModal3 #splice" + (i + 1) + "PosNeg").val();
                splice.elevation_ft = $("#builtupcolumnModal3 #splice" + (i + 1) + "Ft").val();
                splice.elevation_in = $("#builtupcolumnModal3 #splice" + (i + 1) + "In").val();
                splice.elevation_fr = $("#builtupcolumnModal3 #splice" + (i + 1) + "Fr").val();
                splice.profile = $("#builtupcolumnModal3 #splice" + (i + 1) + "WProfile").val();
			...
			if (main.drawShape(main.member_boxColumn, main.s_type, "#e72f5f", main.member_boxColumn.memberProperties.startPoint.x, main.member_boxColumn.memberProperties.startPoint.y))
			{
				...
				dataModel.insertData(main.member_boxColumn); //cruciColumn
			}
	...
	main.drawShape = function(data, type, color, pointX, pointY)
	{
		...
		var lineData = [ 
            ...
            [0, -Length, 0, Length],
            [-Length, Length, Length, Length],
            [-Length, -Length * 0.4, -Length, Length * 0.4],
            [Length, -Length * 0.4, Length, Length * 0.4],
            [-Length, 0, Length, 0]
        ];
		...
		group.uid  = data.id;
		group.left = pointX * scale + gridOffSet * scale - Length;
	...
	main.drawOnElev = function(data, plane)
	{
		...
		if (plane == "XZ")
		{
			...
			if (pointY != $("#depthdrpdwn").val())
				return;
			pointX 		= mapCoordinate(data.memberProperties.startPoint.x,"X",plane) * scale - 1;
		...
		else
		{
			...
			if (data.memberProperties.startPoint.x != $("#depthdrpdwn").val())
				return;
			var pointY= mapCoordinate(data.memberProperties.startPoint.y, "Y", plane) * scale - 1;
