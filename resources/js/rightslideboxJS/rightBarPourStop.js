			...
			$('#chknopaint').change(function() {
				  ...
				  $('#chknopaint').attr('checked', true);
					    ...
					    $.each( wsDropDown.BeamProfile, function(key, value) {
				         ...
				         $(profiles).appendTo('#pourstopprofile'); 
					    ...
					    $.each(wsDropDown.PourStopWeldType, function(index,data) {   
				     		  ...
				     		  $(weldtype).appendTo('#psWeldtype'); 
						     	  ...
						     	  console.log(wsDropDown.MaterialGrade);
				        ...
				        $.each(wsDropDown.PlateThickness, function(index,data) {   
						  ...
						  $.each(wsDropDown.OverlapLength, function(index,data) {   
				    		  ...
				    		  $.each(wsDropDown.Fraction, function(index,data) {   
				    			  ...
				    			  $('<option>').val(data.fr).text(data.fr_fra).appendTo('#pourstopLegLengthFr,#slabEdgeFr,#weldLengthFr,#pitchFr,#switchlength2,#boltSpacingFr');
				    			 
				     		   
				     		});
				    		  
				    		$.each(wsDropDown.Inch, function(index,data) {   
				    			   $('<option>').val(data.Inch).text(data.Inch).appendTo('#pourstopLegLengthIn,#slabEdgeIn,#weldLengthIn,#pitchIn,#switchlength1,#boltSpacingIn');
				    		});
				    				
				    				
				    		 $.each(wsDropDown.SurfacePreparation, function(index,data) {   
				       		  var weldsize = "<option>"+data.SSPC_No+"</option>";
				       		  $(weldsize).appendTo('#pssurfacePreparation1,#psfireProofing1,#psgalvanization1'); 
...
*/	
    ...
    //ref. Drwaing Number
     ...
     $("#pourstopprofile").change(function(){
   	...
   	$("#slabEdgeFt").keyup(function(e){
 		$("#slabEdgeFt").removeClass("importantRed");
 		});
   	$("#weldLengthFt").keyup(function(e){
 		$("#weldLengthFt").removeClass("importantRed");
 		});
   	$("#pitchFt").keyup(function(e){
 		$("#pitchFt").removeClass("importantRed");
 		});
   	$("#boltSpacingFt").keyup(function(e){
 		$("#boltSpacingFt").removeClass("importantRed");
 		});
