...
var WT_Profile = [];
var WT_MaterialGrade = [];

var HSS_Profile = [];
var HSS_MaterialGrade = [];
var HSS_MaterialGrade2 = [];

var screenwallframetable = "";
...
var horizontalshape = "horizontalshapes",
	...
	screenwallframequantity = "screenwallframequantity";
	feet = "feet",
	inches = "Inch",
	fraction = "fr_fra",
	fractiondecimal = "fr", 
...
$(document).ready(function(){	
	...
	screenwallframetable = $('#tableScreenWallFrames').DataTable({	 	       		
		"dom": 'ftipr',
	    "bInfo": false,
	     searching: false,
	     paging: false,
   ...
   jQuery('.dataTable').wrap('<div class="dataTables_scroll" />');
		...
		//Horizontal Shape Dropdown Change Event
        	 ...
        	 if($("#drpHorizontalMemberShape").val() != "")
	 	     {
        		 ...
        		 else if($("#drpHorizontalMemberShape").val() == "WT")
	        	 {
	        		 if(WT_Profile.length == 0 || WT_MaterialGrade.length == 0)
	        		 {
	        			 WT_Profile = getProfiles($("#drpHorizontalMemberShape").val());
	        			 WT_MaterialGrade  = getMaterialGrade("W");
	        		 }
	        		 fillProfileDropdownList('drpHorizontalMemberProfile',WT_Profile);
	        		 fillMaterialGradeDropdownList('drpHorizontalMemberMaterialGrade',WT_MaterialGrade);	        		 
       ...
       //Vertical Shape Dropdown Change Event
        	 ...
        	 if($("#drpVerticalMemberShape").val() != "")
	 	     {
        		 ...
        		 else if($("#drpVerticalMemberShape").val() == "WT")
	        	 {
	        		 if(WT_Profile.length == 0 || WT_MaterialGrade.length == 0)
	        		 {
	        			 WT_Profile = getProfiles($("#drpVerticalMemberShape").val());
	        			 WT_MaterialGrade  = getMaterialGrade("W");
	        		 }
	        		 fillProfileDropdownList('drpVerticalMemberProfile',WT_Profile);
	        		 fillMaterialGradeDropdownList('drpVerticalMemberMaterialGrade',WT_MaterialGrade);	       		 
		...
		{
			var table = $("#tableScreenWallFrames tbody");
			table.find('tr').each(function (i) 
		    {
		        ...
		        if(rowid == selectedrowid)
		        {
	                ...
	                var	HorizontalMemberShape = $tds.eq(2).text(),
	                	...
	                	ScreenWallFrameQuantity = $tds.eq(12).text(), 
	                	
	                    MemberWeightLbs = $tds.eq(13).text(),
	                    MemberWeightTons = $tds.eq(14).text(), 
	                    ConnectionType = $tds.eq(15).text(), 
	                    EndPlateThickness =  $tds.eq(16).text().replace("PL","").replace('"X8 1/2"',''), 
	                    ...
	                    /*WeldLength = $tds.eq(22).text(),*/
	                    
	                    HorizontalLenghtInFeet = $tds.eq(22).text(),
	                    HorizontalLengthInInch = $tds.eq(23).text() == "" ? "0" : $tds.eq(23).text(),
	                    HorizontalLengthInFraction = $tds.eq(24).text()== "" ? "0" : $tds.eq(24).text(),
	                    
	                    VerticalLenghtInFeet = $tds.eq(25).text(),
	                    VerticalLengthInInch = $tds.eq(26).text()== "" ? "0" : $tds.eq(26).text(),
	                    VerticalLengthInFraction = $tds.eq(27).text()== "" ? "0" : $tds.eq(27).text(),
	                    
	                    /*WeldLengthInFeet = $tds.eq(29).text(),                    
	                    WeldLengthInInch = $tds.eq(30).text()== "" ? "0" : $tds.eq(30).text(),
	                    WeldLengthInFraction = $tds.eq(31).text()== "" ? "0" : $tds.eq(31).text(),*/
	                    ReferenceDrawing = $tds.eq(28).text();
		        	...
		        	$('#txtVerticalLengthInFeet').val(VerticalLenghtInFeet);		        	
		        	...
		        	$('#txtScreenWallFrameQuantity').val(ScreenWallFrameQuantity);
		        	...
		        	if(ConnectionType == "End Plate")
		        	{
				        	...
				        	$('#drpEndPlateThickness').val(getOptId('drpEndPlateThickness', EndPlateThickness)).change();
        ...
        function validateForm()
        {
        	...
        	if(validated == true)
        	{
            	...
            	var VerticalLenghtInFeet = $("#txtVerticalLengthInFeet").val();
        ...
        function bindTable()
        {
        	...
        	var ScreenWallFrameQuantity = $("#txtScreenWallFrameQuantity").val();
        	
        	var ConnectionType = $("#ConnectionTypeScreenWall").find("option:selected").text();
        	
        	var BoltGrade="",BoltDia="",NoOfBolts="", BoltDiaDecimal = "", PlateThickness = "";
        	...
        	if(ConnectionType == "End Plate")
        	{
        		PlateThickness = "PL" + $("#drpEndPlateThickness").find("option:selected").text() + '"X8 1/2"';
        	...
        	var VerticalLenght = parseFloat($("#txtVerticalLengthInFeet").val()) * 12 + parseFloat($("#drpVerticalMemberLengthInInch").val()) + parseFloat($("#drpVerticalMemberLengthInFraction").val());
        	...
        	var Total_Member_Weight_Lbs = (HorizontalWeight + VerticalWeight) * parseInt($('#txtScreenWallFrameQuantity').val());
        	...
        	{
        		alert(isEdit);
        		$('#tableScreenWallFrames').dataTable().fnUpdate(
            			['<label class="custom-control custom-checkbox" >'+
            			 ...
            			 RowNoForAddEdit, 
            			 HorizontalShape, HorizontalProfile,HorizontalMaterialGrade, HorizontalTotalLength, HorizontalQuantity,
            			 VerticalShape,VerticalProfile,VerticalMaterialGrade,VerticalTotalLength,VerticalQuantity, ScreenWallFrameQuantity,        			 
            			 Total_Member_Weight_Lbs.toFixed(3), Total_Member_Weight_Tons.toFixed(3),            			          			 
            			 ConnectionType,PlateThickness,BoltGrade, BoltDia,           			 
        	...
        	else
        	{
        		...
        		var DataRow = '<tr id="row' + RowNoForAddEdit +'"><td class="text-center">'+
      					  ...
      					  '</tr>' ;
        		screenwallframetable.row.add($(DataRow)).draw(false);
        	}
        ...
        function constructJsonArrayFromTable() {
            ...
            table.find('tr').each(function (i) 
            {
                var $tds = $(this).find('td'),                    
		                    ...
		                    HorizontalLenghtInFeet = $tds.eq(22).text(),
		                    HorizontalLengthInInch = $tds.eq(23).text(),
		                    HorizontalLengthInFraction = $tds.eq(24).text(), 
		                    
		                    VerticalLenghtInFeet = $tds.eq(25).text(),
		                    VerticalLengthInInch = $tds.eq(26).text(),
		                    VerticalLengthInFraction = $tds.eq(27).text(), 
		                    
		                    ReferenceDrawing = $tds.eq(28).text();
               
               var screenWallFrameArray = {
               ...
               ScreenWallFrames.push(screenWallFrameArray);
            });
            ...
            $.ajax({
    	        ...
    	        dataType : 'JSON',
			    data: { miscsteelgroup:"ScreenWallFrames",
			    	    miscsteeljson:JSON.stringify({"ScreenWallFrames":ScreenWallFrames})
			    	  },			    	
