...
$(document).ready(function(){	
	...
	BridgeAngletable = $('#tbleBridgeAngle').DataTable( {
		"dom": 'ftipr',
	    "bInfo": false,
	     searching: false,
	     paging: false,
	  ...
	  //Gusset Plate Change Event	 
       ...
       $("#drpWeldTypeforGussetPlate").change( function(){
		...
		{
			var table = $("#tbleBridgeAngle tbody");
			table.find('tr').each(function (i) 
		    {
		        ...
		        if(rowid == selectedrowid)
		        {
		        	...
		        	var shape = $tds.eq(2).text(),
                     ...
                     Length = $tds.eq(5).text() == "" ? "0" : $tds.eq(5).text(),
                     ...
                     GussetPlateSize = $tds.eq(11).text() == "" ? "0" :$tds.eq(11).text() ,
		        	...
		        	$('#drpFraction').val(getOptId('drpFraction', LengthInFraction.trim()));		        	
        ...
        function clearFields() {
        	...
        	$('#drpBoltGrade').val(getOptId('drpBoltGrade', BoGrGp)).change();
         	...
         	$('#drpWeldSizeforGussetPlate').val(getOptId('drpWeldSize', weldSizeGP)).change();
        ...
        function validateForm()
        {
        	...
        	if($('#drpGussetPlate').find("option:selected").text() == "Yes")
        	{
            	...
            	{        			
            		...
            		if(validateSize(GussetPlateSize1Inch,GussetPlateSize1Fraction,'Gusset Plate') == false ||
            			validateSize(GussetPlateSize2Inch,GussetPlateSize2Fraction,'Gusset Plate')== false)            		
            			validated =false;
            	}
        ...
        function bindTable()
        {
        	...
        	var GussetPlateThicknessDecimal = "";
        	...
        	if(GussetPlate == "Yes")
        	{
	        	...
	        	GussetPlateThicknessDecimal = $("#drpGussetPlateThicknessInch").val();
        	...
        	if(isEdit == true)
        	{
        	$('#tbleBridgeAngle').dataTable().fnUpdate(
        			['<label class="custom-control custom-checkbox" >'+
        			 ...
        			 WeldSizeDecimal,
        			 BoltDiaDecimal,GussetPlateThicknessDecimal], document.getElementById("row" + editRowNo));
        	}
        	else
        	{
	        	...
	        	var DataRow = '<tr id="row' + RowNoForAddEdit +'"><td class="text-center">'+
			      				...
			      				'<td style="display:none;">'+ GussetPlateThicknessDecimal + '</td>' +
	      					  '</tr>' ;
	        	
	        	//$('#tbleBridgeAngle').append(DataRow);
	        	BridgeAngletable.row.add($(DataRow)).draw(false);
        	}
        ...
        function constructJsonArrayFromTable() {
            ...
            table.find('tr').each(function (i) 
            {
                var $tds = $(this).find('td'),                    
		                    ...
		                    GussetPlateThicknessDecimal = $tds.eq(28).text();
                ...
                var bridgeAngleArray = {			                		
			            			 ...
			            			 [gussetplatesize2] : {[inches]:GussetPlateSize2Inch, 
			               			 ...
			               			 [gussetplatethickness] : {[platethicknessfraction] :GussetPlateThickness.replace('"',''),
			               			 						   [platethicknessdecimal] : GussetPlateThicknessDecimal },
			               			 [weldtype] : WeldType,
            ...
            $.ajax({            	
		        ...
		        dataType : 'JSON',				
			    data: { miscsteelgroup:"BridgingAngle",
			    	    miscsteeljson:JSON.stringify({"BridgeAngle":BridingAngle})
			    	  },			    	
